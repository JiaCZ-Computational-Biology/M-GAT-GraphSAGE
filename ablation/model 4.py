...
best_mse = float('inf')
best_model_file = 'best_gat-graphsage-model.pth'
...
for epoch in range(1000):
    ...
    with torch.no_grad():
        for batch_data, batch_ecfp in test_loader:
            ...
            final_output = combined_model(combined_output)
            batch_targets = batch_data.y.view(-1, 1)
            mse_loss = criterion(final_output, batch_targets)
    ...
    if test_loss_mse < best_mse:
        ...
        print(f"Improved model found at epoch {epoch + 1}, MSE: {best_mse:.4f}")

print(f"Training complete. Best model MSE: {best_mse:.4f}")