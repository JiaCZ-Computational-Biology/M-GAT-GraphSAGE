...
best_model_file = 'D:\\pycharm\\gutingle\\pythonProject2\\突出核蛋白\\best_gat-graphsage-model.pth'
...
with torch.no_grad():
    for batch_data, batch_ecfp in test_loader:
        ...
        all_predictions.append(final_output.numpy())
        all_targets.append(batch_data.y.view(-1, 1).numpy())
...
pearson_corr, _ = pearsonr(all_targets.flatten(), all_predictions.flatten())
