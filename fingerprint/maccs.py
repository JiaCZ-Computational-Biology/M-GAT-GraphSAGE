...
class CNNNet(nn.Module):
    ...
    def forward(self, maccs):
        maccs = maccs.squeeze(1).unsqueeze(1)
        x = self.relu(self.conv1(maccs))
...
for index, row in train_df.iterrows():
    ...
    try:
        ...
        train_data_list.append((data, maccs))
...
cnn_model = CNNNet(input_dim=167, output_dim=167)
combined_model = CombinedNet(input_dim=168, hidden_dim=512, output_dim=1)
...
for epoch in range(1000):
    ...
    for batch_data, batch_maccs in train_loader:
        optimizer.zero_grad()
