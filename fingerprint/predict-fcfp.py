...
def get_fcfp(smiles, radius=2, nBits=1024):
    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        raise ValueError(f"Cannot generate molecule from SMILES: {smiles}")
    fp = AllChem.GetMorganFingerprintAsBitVect(mol, radius, nBits=nBits, useFeatures=True)
...
class CNNNet(nn.Module):
    ...
    def forward(self, fcfp):
        fcfp = fcfp.squeeze(1).unsqueeze(1)
        x = self.relu(self.conv1(fcfp))
...
def evaluate_model(model_path, test_csv_file, smiles_column='Smiles', target_column='pchembl'):
    ...
    print("Processing independent test set...")
    ...
    with torch.no_grad():
        for batch_data, batch_fcfp in test_loader:
            gat_graphsage_output = gat_graphsage_model(batch_data)
            cnn_output = cnn_model(batch_fcfp)
    ...
    print(f"Pearson correlation p-value: {pearson_p:.6e}")
    print("=" * 50)
    ...
    results_file = 'independent_test_predictions.csv'
...
if __name__ == "__main__":
    ...
    if not os.path.exists(model_path):
    ...
    elif not os.path.exists(test_csv_file):
        print(f"Error: Test set file {test_csv_file} does not exist!")
        print("Please provide the correct independent test set file path")
    else:
        ...
        print(f"\nFinal evaluation completed, processed {results['Sample_Count']} samples")
        print(f"Prediction results saved, check file: independent_test_predictions.csv")